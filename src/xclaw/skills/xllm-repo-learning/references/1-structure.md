...
sequenceDiagram
    ...
    Main->>Main: 解析 flag, 推导 backend/model_type/parser
    Main->>Opt: 组装 xllm::Options
    Main->>Master: create_master()
    Master->>Master: 补齐 enable_mla / NPU backend
...
1. `xllm.cpp` 不是纯粹的 `main`，它是启动语义编排器。  
   很多关键语义都在这里被“定死”，比如 `backend` 自动推导、parser 自动选择、`enable_cache_upload`/`enable_kvcache_store` 组合开关；`enable_mla` 现在由 `Master` 构造阶段补齐。
...
- `model_type` 决定能力细节：例如 `enable_mla`、parser、某些模型能力兼容性。
...
`enable_mla` 现在属于 `ModelArgs` 的一部分：`HFModelLoader::load_model_args()` 会根据模型路径调用 `util::should_enable_mla(...)` 写入，`ModelContext` 再通过 `get_model_args().enable_mla()` 暴露给模型和执行层。
...
- `xllm/core/util/model_config_utils.h`
- `xllm/core/util/model_config_utils.cpp`
