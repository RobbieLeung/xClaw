...
- 本地引擎特例
  ...
  - `xllm/core/runtime/dit_worker_impl.h`
...
| 拓扑描述层 | `ParallelArgs` | 保存 rank/world_size/dp/cp/tp/sp/cfg/ep 等拓扑字段和各类 group 指针 |
...
### 2.3 这个模块当前的重点是 TP / DP-local / CP / EP / SP，而不是 PP
...
- `moe_tp_group_`
- `moe_ep_group_`
- DiT 专用的 `dit_tp_group_ / dit_sp_group_ / dit_cfg_group_ / dit_dp_group_`

这里没有 `pp_group_`。而且在 NPU `MappingNPU::Options` 初始化里，`pp_size(1)` 仍是固定配置，`cp_size` 已经从启动参数进入 mapping；`sp_group_` 当前仍别名到 TP group。这说明当前 `parallel_state` 的主要关注点是：

- 常规张量并行 TP；
- DP 下同一 TP slot 之间的局部 gather；
- prefill 侧的 CP 切分与 worker/engine 协同；
...
2. **非 owning 的 group 指针**
   ...
   - `moe_ep_group_`
   - `moe_tp_group_`
   - `dit_tp_group_ / dit_sp_group_ / dit_cfg_group_ / dit_dp_group_`
...
```text
DistManager
  -> 为每个设备准备一个占位 ParallelArgs(rank, world_size, dp_size, cp_size, nullptr, ep_size)
  -> 启 WorkerServer

WorkerServer
  -> 向 master node 同步地址与 rank
  -> 构造 CollectiveCommunicator(global_rank, world_size, dp_size, ep_size, cp_size)
...
2. 构造 `CollectiveCommunicator comm(worker_global_rank, world_size, dp_size, ep_size, cp_size)`；
...
`enable_mla` 现在不再作为 `ModelContext` 构造参数传入，而是由 `HFModelLoader` 写进 `ModelArgs`，模型侧通过 `context.get_model_args().enable_mla()` 读取。
...
torch/HCCL process group 建组代码里：
...
但要注意：CP 加入后，执行侧还会使用 `world_size / (dp_size * cp_size)` 作为 attention/DP driver 语义下的局部 TP 尺寸；NPU `MappingNPU` 里也用这个 CP-aware 的 `attn_tp_size`。
...
- `dp_driver_ = parallel_args.dp_size() > 1 && parallel_args.rank() % (tp_size * cp_size) == 0`，其中 `tp_size = world_size / (dp_size * cp_size)`
...
- 直接创建一个普通 `ParallelArgs(global_rank, world_size, dp_size, cp_size, nullptr, ep_size)`；
- 不在构造阶段灌入 mapping；
...
- 填入 `dp_size / tp_size / moe_tp_size / moe_ep_size / pp_size(1) / sp_size(1) / cp_size`；
