...
- `x_claw` 在同一 `workspace_root` 下只允许一个活跃任务
...
```bash
x_claw status --workspace-root /abs/path/to/workspace_root
```
...
```bash
x_claw status --advise "请不要扩大范围，先把日志链路补齐。"
```
...
- 你可以继续通过 `x_claw status` 观察建议是否已被吸收
...
```bash
x_claw status --approve --comment "方案可接受，继续推进。"
```
...
```bash
x_claw status --reject --comment "回滚这个扩 scope，先只修核心链路。"
```
...
- `x_claw status`
- `x_claw status --advise ...`
- `x_claw status --approve`
- `x_claw status --reject --comment ...`
