...
from .human_io import has_pending_human_advice, publish_review_request, resolve_pending_human_advice
...
def _extract_required_bullet_field(response_text: str, field_name: str) -> str:
    ...
    matches = [match.strip() for match in bullet_pattern.findall(response_text) if match.strip()]
    if len(matches) > 1:
